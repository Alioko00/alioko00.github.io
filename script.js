...
var angryBorderBottom = document.getElementById("angryBorderBottom")
	...
	var stopAngryBorder
var stopAngryBlock
var arrBlackLeft = [0,0]
var arrBlackTop = []
var arrBlackNum = 6
var angrcheck = 1
var stopCircleGreen
var indexstr = 0
var angryBlockRefresh = 0
	ctx.beginPath() /* --ÑÅÒÊÀ-- */
	ctx.lineWidth = 1
...
clearExtr = window.setInterval(extr, 1)
 ...
 // window.setTimeout(circleGreen, 1)
  stopCircleGreen = window.setInterval(circleGreen, 1) /*Äâèæåíèå çåëåíîãî êðóãà*/
...
function maskEffect() {						/*Àíèìàöèÿ çåëåíûé è êðàñíûé ôîí*/
...
function funcTest(){			/*Àíèìàöèÿ âçðûâà*/
...
function circleGreen(){					/*Çåëåíûé êðóã*/
...
function circleMove() { 					/*Äâèæåíèå çåëåíîãî êðóãà*/
...
function func5(){				/*Ñîçäàíèå êîîðäèíàò äâèãàþùèõñÿ äâóõ îãíåíûõ áëîêîâ*/
...
function funcx(){ 					/*Ôóíêöèÿ ãåéìîâåð åñëè ïîïàñòü â áëîê*/
...
function funcNew(){ 				/*Ñîçäàíèå êîîðäèíàò ÷åðíûõ áëîêîâ*/
...
function extr() {						/*Ôóíêöèÿ ãåéìîâåð åñëè ïîïàñòü â êðàñíûå áëîêè ïî áîêàì*/
...
function angryAnima() { 					/*Ôóíêöèÿ êðàñíûõ áëîêîâ ïî áîêàì*/
		if (angryBlockRefresh===0) return;
		angrySide
	var angrySideMin = 1
	var angrySideMax = 9
...
function angryBlock() {						/*Ïðåñëåäóþùèé áëîê*/
